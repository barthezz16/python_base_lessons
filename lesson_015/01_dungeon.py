    ...
    def choose_action(self):
        ...
        if int(self.choice) > 3:
            ...
            if int(self.choice) == 1 and len(self.mob_list) > 0:
                ...
                self.make_a_choice()
                # TODO Нужно уходить от рекурсии в пользу цикла
                # TODO Обернуть выбор действия в цикл - тогда тут, если локация зачищена просто
                # TODO начнётся запрос нового выбора
    ...
    def make_a_choice(self):
        ...
        if int(self.choice) == 3:
            ...
            if int(self.choice) == 1:
                ...
                self.choose_action()
                # TODO Причина тут
                # TODO После убийтсва моба запускается выбор действия
                # TODO И, при выборе 1, если есть мобы, которые живы, функция просто печатает их
                # TODO И больше ничего не делает.
                # TODO Если же после убийства мобов не остаётся - вызывается та ваша рекурсия
                # TODO В идеале было бы чтобы только один метод вызывал другие
                # TODO А то выходит всё запутаннее и запутаннее :)
                # TODO Впору рисовать все зависимости и ходы на бумаге
            if int(self.choice) == 2:
