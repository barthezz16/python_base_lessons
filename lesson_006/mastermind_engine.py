...
def make_number(digits_amount=4):
    ...
    while len(_numbers) < digits_amount:
        ...
        if _numbers[0] == 0:
            _numbers[0] = 1  # Это операция сравнения, а не изменения согласен, это совсем глупая опечатка...
            # TODO Я тоже частенько опечатываюсь, это не страшно, теперь, когда первое число добавляется вне цикла
            # TODO Эту проверку можно убрать, остальное всё верно
    print(_numbers)
    return _numbers
