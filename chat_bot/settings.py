    ...
    'registration': {
        ...
        'steps': {
            ...
            'step2.1': {
                ...
                # вроде получилось, даже вроде как на 1 вариант похоже, но я отдельный handler не добавлял..
                # осталось придумать как это красиво на экран вывести... а то {", ".join(map(str,.......
                # не хочет тут работать..
                #  И почему то если я ввожу город в который нет рейса, получаю сообщение что нет рейса
                #  и список доступных город для прилета, ввожу название нового города,
                #  но оно не меняется и остается стрым
                # TODO Так ведь этот город изменяется не на этом шаге, context['city_arrival'] = text.title()
                # TODO Без этого город и не изменится. Нужно либо эту проверку раньше провести (на втором шаге)
                # TODO Либо что-то ещё придумать)
            ...
            'step4': {
                ...
                'next_step': 'step5'  # не совсем понимаю что тут должго быть...
                # TODO А что здесь не так?
            },
            'step5': {
